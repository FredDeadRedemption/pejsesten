...
async fn main() {
    ...
    loop {
        ...
        let deck_builder = match &mut screen {
            Screen::DeckBuilder(db) => {
                db.tick_clipboard(get_frame_time());
                Some(deckbuilder_ui::run(db))
            }
            _ => None,
        };
        ...
        match &screen {
            ...
            Screen::DeckBuilder(db) => {
                ...
                if let Some(frame) = &deck_builder {
                    render::draw_catalog_cards(db, &frame.cards, &cache);
                }
            }
        ...
        // egui paints last so its panels cover any catalog art bleeding past the scroll area
        if let Some(frame) = deck_builder {
            egui_macroquad::draw();
